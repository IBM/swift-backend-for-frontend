...
function killServerProcesses() {
    pid="$(pgrep swiftbackendforfrontend)"
    if [ "$pid" != "" ]
    then
        echo "killing swiftbackendforfrontend"
        kill $pid
    fi
